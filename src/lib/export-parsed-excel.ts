...
function writeA3Header(worksheet: WorkSheet, data: ParsedExcel): void {
  const { metadata } = data;

  setCellValue(worksheet, A3_LABEL_CELLS.company, A3_HEADER_LABELS.company);
  setCellValue(worksheet, A3_METADATA_CELLS.companyCode, metadata.companyCode);
  setCellValue(worksheet, A3_METADATA_CELLS.companyName, metadata.companyName);

  setCellValue(worksheet, A3_LABEL_CELLS.selection, A3_HEADER_LABELS.selection);
  setCellValue(worksheet, A3_METADATA_CELLS.selection, metadata.selection);

  setCellValue(worksheet, A3_LABEL_CELLS.date, A3_HEADER_LABELS.date);
  setCellValue(worksheet, A3_METADATA_CELLS.exportDate, metadata.exportDate);
...
function writeA3Table(
  ...
  data.columns.forEach((column, columnIndex) => {
    ...
    setCellValue(worksheet, address, column);
  });
  ...
  data.rows.forEach((row: ExcelRow, rowIndex) => {
    ...
    data.columns.forEach((column, columnIndex) => {
      ...
      setCellValue(worksheet, address, row[column] ?? null);
    });
  });
